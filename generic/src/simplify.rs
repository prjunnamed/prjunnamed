...
pub fn simplify(design: &mut Design) -> bool {
    let rules = netlist_replace! {
        ...
        // NOTE: the following rule is useful because this exact pattern is
        // generated by the lowering of assign cells. The commutative variations
        // of this rule don't have this property and turn out to be counter-productive.
        [PMux [PAny@x]   [PAny@a]   [PMux [PAny@y] [PAny@b] [PAny@c]]]  if a == b =>
            design.add_mux(design.add_or1(x, y), a, c);
        /*
        [PMux [PAny@x]   [PAny@a]   [PMux [PAny@y] [PAny@b] [PAny@c]]]  if a == c =>
            design.add_mux(design.add_or1(x, design.add_not1(y)), a, b);
        [PMux [PAny@x]   [PMux [PAny@y] [PAny@a] [PAny@b]] [PAny@c]]    if a == c =>
            design.add_mux(design.add_or1(design.add_not1(x), y), a, b);
        [PMux [PAny@x]   [PMux [PAny@y] [PAny@a] [PAny@b]] [PAny@c]]    if b == c =>
            design.add_mux(design.add_and1(x, y), a, b);
        */
...
mod test {
    ...
    fn test_mux_fold() {
        for size in [1, 2] {
            ...
            assert_simplify!(
                |ds| {
                    let a = ds.add_input("a", size);
                    let b = ds.add_input("b", size);
                    ds.add_mux(ds.add_input1("x"), &a, ds.add_mux(ds.add_input1("y"), &a, b))
                };
                [PMux [POr [PInput ("x")] [PInput ("y")]] [PInput ("a")] [PInput ("b")]] => true;
            );
        }
    }
